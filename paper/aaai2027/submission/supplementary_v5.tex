...
\begin{figure}[t]
\centering
\includegraphics[width=0.9\columnwidth]{figures/fig2_integration.pdf}
\caption{Integration summary across substrates (moved from the main paper). Discrete grids (Manhattan anchor): additive learned magnitude is at or below baseline while the same trained predictions reduce expansions by 6--15\% as a within-band ranker at $w{=}1.0$. Continuous roadmaps (Euclidean anchor): additive guidance reduces matched expansions by 15--48\%; the plotted focal $w{=}1.1$ range is an exploratory, post-hoc operating point and is not used for any main-paper claim.}
\label{fig:integrationsupp}
\end{figure}
...
\caption{Compositional-mission hierarchy test (\stage{C11}); $K$ here is mission length. Left: the exact oracle's ratio over a leg-sum baseline falls as missions deepen (headroom exists). Right: learned HRM halting uses \emph{fewer} steps at $K{=}8$; the annotation reports both analysis grains (map-level $\rho=-0.578$, $p=5\times10^{-5}$, $n{=}225$ maps; record-level $\rho=-0.407$, $n{=}675$).}
\label{fig:c11}
\end{figure}
...
The anonymized artifact archive submitted with this paper contains: (i) the analysis scripts that regenerate every main-paper figure and quoted map-level statistic from raw rows (the fixed-provider dynamic reanalysis, the map-clustered reanalysis, the weighted-A\textsuperscript{*} analysis with its exact McNemar and ratio-CI computations, the budget-curve derivation, the reachable-label recount, and all figure generators, including the factorial figure); (ii) the raw paired evaluation rows behind every quoted quantity (\stage{C7}/\stage{C8} evaluation tables, including the fresh-cohort and three-seed replication rows, the weighted-A\textsuperscript{*} development and confirmation rows with the tuned-weight manifest, \stage{C9}/\stage{C9h}/\stage{C9b}/\stage{C10} evaluation tables, \stage{C11} evaluation and halting tables, \stage{C13} development and confirmation rows, and the \stage{C14} per-cell outputs with sampled-index manifests and the measured $w_{\min}$ stream); (iii) map/cohort manifests, seed lists, frozen configurations, the binding-budget calibration file, and the integrity manifests for the frozen confirmations; (iv) the preregistration design documents from \stage{C7} onward, including the \stage{C14} design with its pre-execution amendment and the post-completion supersession record; and (v) a top-level README with reproduction commands and expected outputs. The archive's README lists exactly which checkpoints are included (the fixed dynamic provider and the frozen adaptation sources) and which are regenerable from the provided commands and seeds.
