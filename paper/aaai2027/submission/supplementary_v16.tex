%File: supplementary_v16.tex --- Technical Appendix (AAAI-27 anonymous submission, final; includes Section L: scale and external-map studies)
...
\bottomrule
\end{tabular}
\caption{\stage{C8-X} summary on the 21 held-out maps (ZS = zero-shot; scratch row = highest observed street success). $^{*}$: map-clustered 95\% CI excludes zero. B2 = adapted($M{=}8$) minus the mean over all eight $M{=}1$ draws. $^{\dagger}$: original endpoints, inference computed in the repaired eight-test family; balanced B2 rows are post-outcome; the repaired family does not inherit preregistered status.}
