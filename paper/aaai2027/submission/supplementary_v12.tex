%File: supplementary_v12.tex --- Technical Appendix (AAAI-27; v11-review response: marginal-interval language, matched path quality, C11 exact configurations)
...
\textbf{\stage{C10} composition.} Map-paired deltas vs.\ zero-shot: four cells $\approx$0 with CIs crossing zero; the two ON-LSTM rooms cells are \emph{worse} with CIs excluding zero ($+0.089$ $[+0.061,+0.121]$ and $+0.098$ $[+0.076,+0.123]$), matching the published point deltas and upgrading the null to ``never better, sometimes worse.''
