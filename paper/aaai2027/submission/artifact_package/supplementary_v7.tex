%File: supplementary_v7.tex --- Technical Appendix (AAAI-27; consistency revision: extended WA* grid, SIPP table, coverage contrasts, joint-set path quality)
...
\textbf{Extended-grid check (frozen design 2026-07-26; specified post hoc).} Because maze, dense maze, and spiral selected the grid maximum $w_h{=}5$, the grid was extended to $w_h \in \{7, 10\}$: the two new weights were run on the development cohort only, the identical frozen selection rule was re-applied over the merged grid, and only suites whose selection changed were evaluated once on the frozen confirmation cohort. Five selections are unchanged---in particular spiral's development success is 0.80 at $w_h{=}5$ and no larger weight beats it, so its confirmation advantage (15/0 discordant) is grid-robust---while dense maze re-selects $w_h{=}7$ (development success 20/20 versus 18/20 at $w_h{=}5$; the development curve is monotone: 1/2/2/4/12/18/20/20 successes of 20 across the eight weights). At $w_h{=}7$ on the confirmation cohort, weighted A\textsuperscript{*} solves 45/50 versus 35/50 for both the frozen $w_h{=}5$ rows and the learned heuristic (15/5 discordant in each pairing, exact $p{=}0.041$, single post-hoc comparisons), with matched-solved learned-to-weighted expansions at parity (median 1.172 $[0.948, 1.479]$, $n{=}30$) and cheaper weighted search on jointly solved maps (mean 843 versus 1{,}217 expansions at $w_h{=}5$). The dense-maze effort comparison in Table~\ref{tab:wastar} is therefore grid-limited: under stronger inflation the control overtakes the learned arm on this suite, and the learned advantage concentrates where no tested inflation closes the gap (spiral). Raw development/confirmation rows and the selection report ship in the artifact package.
...
\textbf{Wall-time decomposition (dynamic arms).} On a 5-map-per-suite CPU sample at binding budgets: building the learned heuristic table (raster/feature construction plus $t_{\max}{+}1$ U-Net forwards) averages 1.23\,s per map (range 0.95--1.59\,s; one forward is ${\sim}5.9$\,ms, so inference is roughly half), and the learned search itself 0.31\,s---about 1.54\,s end to end, versus 0.39\,s for tuned weighted A\textsuperscript{*} and 0.76\,s for the anchor. At this graph scale, fewer expansions do not translate into lower wall time for the learned method; the dynamic result is a search-effort and success result, not a latency result (single-process CPU; script and raw timings in the artifact package).

\textbf{SIPP baseline (frozen design 2026-07-25).} Discrete-time Safe Interval Path Planning on the identical substrate: per-node safe intervals are maximal runs of wait-safe steps under the exact wait predicate (plus pass-through singletons, matching the space--time graph, where arrival never requires a node check), successors take the earliest feasible departure per (edge, target interval) under the exact edge predicate, and the heuristic is the anchor $h_0$. \emph{Correctness gate, pre-committed:} on every instance, SIPP's earliest arrival must equal the backward space--time Dijkstra optimum; all 300 confirmation instances pass, and solvability agrees everywhere. Results are tabulated in Table~\ref{tab:sipp}: run unbudgeted, SIPP solves 0.98--1.00 per suite at optimal path quality, and its interval-state expansions (a different unit from $(v,t)$ expansions; never merged) have small per-suite medians. Per the design's pre-committed framing: the learned result is search-effort structure within the budgeted time-expanded formulation, not superiority over a dedicated dynamic planner on this deterministic substrate. Raw rows with per-instance gate checks and timings ship in the artifact package.
...
\bottomrule
\end{tabular}
\caption{Direct distributed$-$concentrated success contrasts, all 30 factorial cells: paired per-map differences with adaptation seeds averaged within maps and 10k map-bootstrap 95\% CIs (static $n{=}30$ maps, dynamic $n{=}20$). $^{*}$: interval excludes zero (marginal; no multiplicity adjustment). For full fine-tuning and scratch the significant cells are exactly those where concentration forces at most two distinct worlds ($w_{\min}$ in parentheses).}
\label{tab:c14cov}
\end{table*}
...
The anonymized artifact archive submitted with this paper contains: (i) the analysis scripts that regenerate the quoted map-level statistics from raw rows (the fixed-provider dynamic reanalysis, the map-clustered reanalysis, the weighted-A\textsuperscript{*} analysis with its exact McNemar, ratio-CI, and matched path-quality computations, the budget-curve derivation, the reachable-label recount, the factorial analysis, and the MovingAI external-benchmark analysis, plus all figure generators); (ii) the raw paired evaluation rows behind every quoted quantity (\stage{C7}/\stage{C8} evaluation tables, including the fresh-cohort and both retrained-pipeline replication rows, the weighted-A\textsuperscript{*} development and confirmation rows with the tuned-weight manifest and the extended-grid development/confirmation rows with their selection report, the SIPP baseline rows with per-instance gate checks and timings, the MovingAI external rows with the map manifest, the wall-time decomposition sample, \stage{C9}/\stage{C9h}/\stage{C9b}/\stage{C10} evaluation tables, \stage{C11} evaluation and halting tables, \stage{C13} development and confirmation rows, the \stage{C14} per-cell outputs with sampled-index manifests, the measured $w_{\min}$ stream, and the 30-cell coverage-contrast outputs, and the \stage{C14-R} independent-draw replicates); (iii) map/cohort manifests, seed lists, frozen configurations, the binding-budget calibration file, and the integrity manifests for the frozen confirmations; (iv) the preregistration design documents from \stage{C7} onward, including the \stage{C14} design with its pre-execution amendment and post-completion supersession record, the 2026-07-25 SIPP, MovingAI, and \stage{C14-R} designs, and the 2026-07-26 extended-grid design with its disclosed chronology; (v) the two frozen source checkpoints the headline claims rest on (the fixed dynamic provider and the frozen static adaptation source), with all other checkpoints regenerable from the recorded commands and seeds; and (vi) a top-level README with reproduction recipes and expected outputs (the README states explicitly that analysis-script path constants must be pointed at the package's raw directories when running standalone).
