...
\textbf{Independent world-draw replicates (C14-R; frozen design 2026-07-25).} Each original cell used one deterministically sampled world set; C14-R re-draws the world streams independently (draw offsets entering only the pool seeds; test cohorts, budgets, and sampling rules unchanged) at the decisive cells: dynamic $N \in \{1024, 16384\}$ and static $N{=}256$, both coverage levels, all three methods, one optimization seed, 18 arms per draw on the same frozen test cohorts. Draw 3 results (deltas vs the frozen source, map-level CIs): the \emph{direct coverage contrasts replicate}---distributed$-$concentrated is positive with CIs excluding zero for full fine-tuning and scratch everywhere (dynamic $N{=}1024$: $+0.400$ $[+0.200,+0.600]$ and $+0.450$; $N{=}16{,}384$: $+0.300$ and $+0.500$; static $N{=}256$: $+0.133$ $[+0.033,+0.267]$ and $+0.367$)---and static concentrated harm replicates (full FT $-0.233$ $[-0.400,-0.100]$; scratch $-0.500$). Two preregistered readouts deviate, reported as-is: dynamic concentrated $N{=}1024$ full fine-tuning reproduces in magnitude ($-0.250$) but its CI grazes zero ($[-0.500, 0.000]$; scratch $-0.300$ $[-0.550,-0.050]$ passes), so strict R1 fails at $n{=}20$ with one optimization seed; and one LoRA cell degrades significantly under the new draw (static concentrated $N{=}256$: $-0.133$ $[-0.267,-0.033]$), failing R4---the first significant LoRA degradation in the program, still the least-degraded method in its cell. The replicate therefore supports the coverage mechanism while converting LoRA's protection from ``never observed to fail'' to ``most stable, not absolute.'' A second independent draw is in flight and will be reported in the artifact package alongside these rows.
