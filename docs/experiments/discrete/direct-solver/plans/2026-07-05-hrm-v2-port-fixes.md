...
**Audit cross-refs:** `docs/experiments/discrete/direct-solver/audits/PORT_FIDELITY_AUDIT.md` §B1/B2 (attention), §C2 (sparse-emb), §D1–D6 (training), §F (fix list). Original ground truth: `models/hrm/hrm_act_v1.py`, `models/layers.py:98-135` (attention), `models/losses.py` (ACTLossHead), `models/sparse_embedding.py`.
...
**Files:** run artifacts under `HRM-v2/checkpoints/`, report `docs/experiments/discrete/direct-solver/results/RETRAIN_RESULTS.md`, status update in `docs/experiments/discrete/direct-solver/audits/PORT_FIDELITY_AUDIT.md`.
...
- [ ] **Step 4: writeup + commit.** `docs/experiments/discrete/direct-solver/results/RETRAIN_RESULTS.md`: config, curves (steps vs exact-acc/avg-steps/q_halt-acc), gate verdicts, before/after table vs the broken run and the paper. Append a status block to `docs/experiments/discrete/direct-solver/audits/PORT_FIDELITY_AUDIT.md` ("fixes landed @ SHAs; revalidation result"). Commit docs (checkpoints stay untracked).
