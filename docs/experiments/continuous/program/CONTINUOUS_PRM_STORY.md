...
**Per-phase detail:** [`C6_RESULTS.md`](../c06/results/C6_RESULTS.md) · [`C7_RESULTS.md`](../c07/results/C7_RESULTS.md) · [`C8_RESULTS.md`](../c08/results/C8_RESULTS.md) · [`C9_RESULTS.md`](../c09/results/C9_RESULTS.md) · [`C9H_RESULTS.md`](../c09h/results/C9H_RESULTS.md) · [`C10_RESULTS.md`](../c10/results/C10_RESULTS.md) · [`C9B_RESULTS.md`](../c09b/results/C9B_RESULTS.md) (transfer under dynamics — follow-up to C8+C9)
**Program history:** [`EXPERIMENT_RESULTS_COMPENDIUM.md`](../../discrete/learned-heuristic/results/EXPERIMENT_RESULTS_COMPENDIUM.md)
...
## Follow-up: C9b — transfer under dynamics ([`C9B_RESULTS.md`](../c09b/results/C9B_RESULTS.md))
...
## 2026-07-17 addendum: C13 answers the current-state objection

C13 revisits the heuristic program under a stricter condition from the professor meeting: the learned value for a search state may use bounded observations around that state, but not a complete occupancy raster, global obstacle summary, or shortest-path label.

The result is positive after a long mechanism sequence. On 144 untouched worlds across all six static C7 suites, the fixed local-Bellman current-state arm averages **68.31 expansions** versus **81.26** for complete-map field HRM: paired delta **-12.96**, bootstrap 95% CI **[-16.30, -9.74]**, with 109 wins, 3 ties, and 32 losses. All six suite means are negative.

The path-quality comparison is also favorable on this cohort: current mean/max graph-optimal ratios are **1.0235/1.1624**, versus **1.0311/1.3346** for field HRM. A separate reopening-FOCAL current-state control at `w=1.10` returns all 144 valid paths with zero observed bound or certificate violations.

The mechanism matters more than the backbone:

1. Exact behavior returns and exact shallow local-escape targets fail.
2. A one-suite learned local heuristic confirms on maze but fails the six-suite C7 distribution.
3. Suite-balanced training alone still fails when inserted statically.
4. One radius-bounded Bellman backup at inference converts the same model from clearly worse to competitive, and fixed alpha 1.50 then confirms on a disjoint cohort.

This changes the earlier “substrate has no headroom” diagnosis in a narrow but useful way. There was still headroom for **changing the information/target/integration contract**, even though repeated backbone and adapter variations had saturated.

- It does not revive a hierarchy claim: the confirmed model is a flat MLP.
- It does support a state-conditioned planning claim under a bounded local-subgraph contract.
- It shows that a method with less global information can beat complete-map learned providers in search expansions when local computation is integrated correctly.

The result does **not** establish formal safety for the direct arm or end-to-end speed. Its unoptimized Python feature construction averages 5.14 seconds/world versus 0.37 for field HRM; model inference and the Bellman backup are cheap, but representation construction must be vectorized or made lazy.

Canonical evidence: [`C13F_M_CURRENT_STATE_RESULT.md`](../c13/results/C13F_M_CURRENT_STATE_RESULT.md).

## Current status

C6-C10/C9b establish learned-heuristic and transfer gains with hierarchy/temporal/interpolation negatives. C11-C12 close the broad hierarchy/depth hypotheses. C13 now adds a confirmed current-state/local-computation result over complete-map C7 providers, with explicit path-quality, bound, known-PRM, and wall-time caveats.
