...
## 2026-07-17 strategic update after C13-M

C13-M invalidates one part of the older recommendation: another method change on the 2-D substrate was not automatically futile. A carefully constrained change to target and planner integration found a real remaining frontier. The winning arm uses bounded observations of each search state plus one radius-bounded Bellman backup and beats complete-map C7 providers in pooled expansions at better empirical path quality.

The ranking of next work is therefore:

1. **Bank and harden C13-M.** Replicate model seeds and a second 144-world cohort; test 211-node and larger roadmaps without changing the frozen operating point.
2. **Fix representation latency.** Vectorize or lazily construct the bounded rays/features, require bit-identical ranks/search outcomes, and remeasure end-to-end time.
3. **Then return to higher-headroom substrates.** C11/C12 still do not support a hierarchy/depth advantage, so graph-native or high-dimensional tasks remain the right architecture question after C13 is hardened.

Do not retune alpha on the C13-M cohort. It is confirmation data. Any new operating point requires a new development block and another untouched confirmation.

For publication, present C13 as an explicit Pareto frontier:

- the direct alpha-1.50 arm is the confirmed matched-quality expansion result;
- the reopening `w=1.10` FOCAL arm is the slower bounded-safety point; and
- neither arm currently supports an end-to-end latency claim.
