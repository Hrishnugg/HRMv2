...
def optima():
    """Per-world optimal arrival from the canonical fresh raw (any solved
    row carries the world's backward-Dijkstra optimum)."""
    out = {}
    p = os.path.join(RUNS, "c8r_fresh_eval", "results",
                     "continuous_prm_c8_eval_raw.csv")
    with open(p, newline="") as f:
        for r in csv.DictReader(f):
            if (r["suite"] == SUITE and r["mode"] == "astar"
                    and r["found"] in ("True", "true", "1")):
                try:
                    o = float(r["optimal_arrival"])
                except ValueError:
                    continue
                if np.isfinite(o) and o < 1e29:
                    out[int(float(r["world_index"]))] = int(o)
    return out
...
def main():
    ...
    # Path quality at w=7 on the same jointly solved maps (optima from the
    # canonical raw), plus the paired blind-minus-w7 suboptimality difference.
    opt = optima()
    jq = [w for w in joint if w in opt]
    w7_sub = np.array([w7[w][2] / opt[w] for w in jq])
    bl_sub = np.array([bl[w][2] / opt[w] for w in jq])
    d = bl_sub - w7_sub
    reps = [float(np.mean(d[rng.integers(0, len(d), len(d))]))
            for _ in range(10_000)]
    rep["path_quality_joint"] = dict(
        n=len(jq), w7_subopt_mean=float(np.mean(w7_sub)),
        blind_subopt_mean=float(np.mean(bl_sub)),
        diff_mean=float(np.mean(d)),
        diff_ci=[float(np.percentile(reps, 2.5)),
