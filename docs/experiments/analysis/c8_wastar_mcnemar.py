...
def main():
    ...
    for suite in SUITES:
        ...
        # Matched path quality: jointly solved maps with known optimum, plus
        # the paired per-map suboptimality difference (learned - weighted).
        joint = [w for w in worlds
                 if wa[(suite, w)][0] and bl[(suite, w)][0] and (suite, w) in opt]
        wa_sj = [wa[(suite, w)][2] / opt[(suite, w)] for w in joint]
        bl_sj = [bl[(suite, w)][2] / opt[(suite, w)] for w in joint]
        d_sub = [b - a for b, a in zip(bl_sj, wa_sj)]
        rows[suite] = {
            "n_maps": len(worlds),
            ...
            "joint_n": len(joint),
            "wastar_subopt_joint": float(np.mean(wa_sj)) if wa_sj else None,
            "blind_subopt_joint": float(np.mean(bl_sj)) if bl_sj else None,
            "subopt_diff_joint_mean": float(np.mean(d_sub)) if d_sub else None,
            "subopt_diff_joint_ci": boot_ci(d_sub, np.mean, rng),
            "w_h": tuned[suite]["w_h"],
        }
    qs = bh(ps)
    ...
    lines += [
        ...
        "",
        "| Suite | joint n | Subopt learned (joint) | Subopt WA* (joint) | "
        "Paired diff learned-WA* [CI] |",
        "|---|---|---|---|---|",
    ]
    for suite in SUITES:
        r = rows[suite]
        ci = r["subopt_diff_joint_ci"]
        cis = "n/a" if ci is None else f"[{ci[0]:+.3f},{ci[1]:+.3f}]"
        lines.append(
            f"| {LABELS[suite]} | {r['joint_n']} "
            f"| {r['blind_subopt_joint']:.3f} | {r['wastar_subopt_joint']:.3f} "
            f"| {r['subopt_diff_joint_mean']:+.3f} {cis} |")
