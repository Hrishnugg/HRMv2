...
**Snapshot:** base inventory 2026-07-10; C12 findings refreshed 2026-07-14; C13-M confirmation plus C13-N/C13-O HRM diagnostics refreshed 2026-07-19
...
- Authored designs, plans, result reports, audits, and historical notes through the completed C13-M confirmation and C13-N/C13-O architecture diagnostics.
- **53 generated continuous-run Markdown reports** and **5 generated discrete Modal-survey reports** at the 2026-07-19 refresh.
...
**Sources:** [C13 design](continuous/c13/design/2026-07-16-c13-state-conditioned-heuristic-design.md), [initial audit](continuous/c13/results/C13_INITIAL_AUDIT.md), [pipeline smoke](continuous/c13/results/C13B_ROLLOUT_RANKER_SMOKE.md), [identifiability study](continuous/c13/results/C13B_IDENTIFIABILITY_STUDY.md), [independent-certifier gate](continuous/c13/results/C13C_CERTIFIED_SEARCH.md), [shared-queue oracle gate](continuous/c13/results/C13D_SHARED_QUEUE_ORACLE.md), [shared-queue exact-target gate](continuous/c13/results/C13E_SHARED_QUEUE_EXACT_TARGET.md), [current-state literature/target decision](continuous/c13/design/2026-07-17-c13-current-state-literature-and-next-target.md), [C13-M preregistration](continuous/c13/design/2026-07-17-c13m-matched-quality-confirmation.md), [C13-F through C13-M canonical result](continuous/c13/results/C13F_M_CURRENT_STATE_RESULT.md), [C13-N preregistration](continuous/c13/design/2026-07-17-c13n-hrm-substitution.md), [C13-N result](continuous/c13/results/C13N_HRM_SUBSTITUTION_RESULT.md), [C13-O preregistration](continuous/c13/design/2026-07-17-c13o-hrm-summary-last-alignment.md), and [C13-O result](continuous/c13/results/C13O_HRM_ALIGNMENT_RESULT.md).
**Status:** complete benchmark-level current-state result; C13-M primary matched-quality and bounded-control gates pass, while the follow-on C13-N substitution and C13-O summary-last development gates fail.
...
**Decision.** Treat C13-M as the completed local result with explicit Pareto framing: the alpha-1.50 direct arm is the confirmed matched-quality expansion result, while the `w=1.10` reopening-FOCAL arm is the slower bounded control. Treat C13-N and C13-O as negative architecture diagnostics, not contradictions of C13-M or general HRM rejections. Do not retune either failed development block or open confirmation. If HRM is pursued, preregister a persistent-state pilot whose carry occurs across causally ordered local Bellman/search events, includes a matched zero-reset arm and stationary-target control, and keeps current-state inputs and queue semantics explicit. Before external publication, optimize feature construction, add model-seed and cohort replication, and review the exact known-PRM/current-local-subgraph wording with the professor.
...
5. **Recompute C11 inference at the TEST-world level.** The main grid and 12-run scaled addendum are complete, but repeated model seeds are nested within 25 TEST worlds; publication intervals must cluster or aggregate at world level.
