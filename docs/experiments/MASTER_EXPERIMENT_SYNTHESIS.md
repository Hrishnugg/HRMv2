...
- **56 authored experiment documents**, including designs, plans, result reports, audits, historical notes, and one external background paper.
- **37 generated continuous-run Markdown reports** and **5 generated discrete Modal-survey reports**.
- The raw-result layer beneath the reports: **2,772 continuous artifacts** (about 25.99 GB / 24.20 GiB) and **16,564 discrete artifacts** (about 105.7 MB / 100.8 MiB) at the snapshot time.
...
| Hierarchical models are superior planning heuristics            | **Not supported on tested formulations**  | [C11 results](continuous/c11/results/C11_RESULTS.md), [program audit](cross-space/PROGRAM_AUDIT_HIERARCHY_AND_SUBSTRATE.md), [head-to-head](cross-space/HRM_HEADTOHEAD.md) | Do not generalize to all planning; C11 still uses scalar regression, while persistent hidden state and tied full-graph refinement remain untested. |
...
**Caveats.** Dense maze is outside a useful calibration band and has matched n=1; rooms-large Euclid success 0.75 is above the intended band. Heavy results are local and single-seed. Field ON-LSTM, more seeds, cluster confirmation, and a success-aware composite remain open.
...
**Scaled addendum.** The separate 12-run `unet_film_big` versus `hrm_trace_big` addendum tests whether 10–20M-parameter scale changes the shallow ranking or escapes HRM collapse. It has 8/12 checkpoints at this snapshot, is still training, and contributes no evaluation result yet.

### C12 planned follow-up: persistence rather than another scalar backbone

**Sources:** [C12 design](continuous/c12/design/2026-07-10-c12-persistent-hierarchical-planning-design.md) and [implementation plan](continuous/c12/plans/2026-07-10-c12-persistent-hierarchical-planning.md).
**Status:** draft design/methods only; no C12 evidence exists.

C12-A proposes hidden slow regimes composed with fast dynamics, shared-information snapshot/LSTM/Transformer/ON-LSTM/streaming-HRM controls, persistent carry across closed-loop replans, and world-clustered inference. A mandatory G0-A probe must first prove that identical present observations can imply different futures/actions and that history improves planning. C12-B separately proposes the mechanism C11 intentionally omitted: a weight-tied full-product-graph refiner with deep supervision at cycles 1/2/4/8, compared with both parameter-matched shallow and compute-matched untied graph controls. The tracks are independently gated so one cannot rescue the other post hoc.
...
The audit correctly identified the scalar-regression formulation as a likely bottleneck, but completed C11 now sharpens the conclusion. Composition, non-local structure, an explicit MLP, global U-Net/GNN views, and faithful HRM-v2 ACT still produce no hierarchy/depth dose-response. Architecture is not irrelevant—global-input models separate at shallow K—but the advantage tracks information access and fit, then disappears as depth raises error for every arm. C12 therefore moves to two still-untested mechanisms: persistent state under partially observed slow/fast dynamics and tied full-graph iterative refinement.
...
1. **Recompute C9/C9h/C9b/C10 uncertainty at the TEST-world level.** Adaptation seeds are repeated measurements nested within the same worlds. Report world-clustered bootstrap intervals and direct paired contrasts.
...
5. **Recompute C11 inference at the TEST-world level and complete the scaled addendum.** The main evaluation is complete, but repeated model seeds are nested within 25 worlds and the 12-run scale diagnostic is still live.
...
7. **Correct document arithmetic and stale labels.** In particular HRM-v2 sample exposure, separation of C11's completed 198-run main grid from its live 12-run addendum, C6 “in progress,” and outdated universal-success language.
...
- C11 contributes a completed negative architecture/depth result, but its pooled `world × seed` uncertainty needs world-clustered reanalysis and its scaled addendum is unfinished.

# Paper-scaffolding implications

## Evidence-safe working thesis

> Learned heuristics can substantially reduce search effort and transfer across planning families, but the benefit depends more on representation, planner integration, information access, and supervision regime than on nominal model hierarchy. Strong pooled priors are robust; low-rank specialization, explicit future-window inputs, and adapter interpolation add little once the cheap signal is already captured. Even with large compositional oracle headroom, C11 finds only shallow global-input advantages—not a hierarchy or depth-of-compute dose-response—and recurrent/ACT training is fragile.
...
7. Close with C11: first establish its oracle headroom, then report the negative G1/G2 dose-response result, the shallow global-input separation, collapse diagnosis, and C12's preregistered persistence/refinement follow-up.
...
5. Does the C11 scaled U-Net/HRM addendum change the shallow ranking or escape recurrent collapse?
6. After its G0 authorization probes, does C12-A persistent memory improve closed-loop planning under hidden slow/fast dynamics, or does C12-B tied refinement improve with compute on deep product graphs?
7. Which claims should remain one coherent paper, and which—C12, HRM-v2 fidelity, or early world forecasting—belong in an appendix or separate follow-up?
...
| [2026-07-10-c12-persistent-hierarchical-planning-design.md](continuous/c12/design/2026-07-10-c12-persistent-hierarchical-planning-design.md) | Draft design / methods only | Separately gates persistent hidden-regime dynamics and tied product-graph refinement; no result evidence.                    |
| [2026-07-10-c12-persistent-hierarchical-planning.md](continuous/c12/plans/2026-07-10-c12-persistent-hierarchical-planning.md) | Draft implementation plan | Sixteen test-first tasks, sealed splits, matched compute controls, world-clustered analysis, and compute stop rules.          |


# Generated report disposition

Generated reports are listed individually here so smoke, diagnostic, canonical, and duplicate outputs cannot be mistaken for separate replications. The [continuous catalog](continuous/GENERATED_EVIDENCE.md) and [discrete catalog](discrete/GENERATED_EVIDENCE.md) remain the shorter operational indexes.

## Continuous generated reports (37)
...
| C11 scaled addendum     | `[c11_big/manifest.json](../../hrm-cloud/continuous_prm/runs/c11_big/manifest.json)` plus training logs/checkpoints | Live 12-run scale diagnostic, 8/12 checkpoints at snapshot; no evaluation/result claim. |
...
6. stamping the snapshot time, especially for the C11 scaled addendum, C12, or other live experiments.
