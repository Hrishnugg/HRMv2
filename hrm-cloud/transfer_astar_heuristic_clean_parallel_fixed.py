...
def _init_model_for_stage(model_name: str, arm: str, stage_id: str, device: str) -> Tuple[CleanHeuristicModel, BackboneConfig, int]:
    ...
    if arm == "lora" and stage_idx > 0:
        rank = cfg.lora_rank
        _apply_stacked_lora(model, rank, LORA_ALPHA, stage_idx, include_conv=LORA_ON_CONV, include_attn=LORA_ON_ATTN, init_scale=LORA_INIT_SCALE)
...
def _load_previous_stage_weights(model: nn.Module, arm: str, model_name: str, stage_id: str) -> None:
    ...
    for k, v in ckpt_sd.items():
        ...
        filtered_sd[k] = v
    if skipped:
        print(f"  [load][{arm}][{model_name}][{stage_id}] skipped {skipped} keys with shape mismatches (stacked LoRA growth)")
...
def run_pipeline(train_models: List[str], eval_models: List[str], train_modes: List[str], eval_modes: List[str],
    ...
    print(f"Train epochs:    { {s.stage_id: s.train_epochs for s in STAGES_TO_RUN} }")
        ...
        if SKIP_TRAIN:
            ...
            if STAGE_INDEX[stage.stage_id] == 0:
                if train_models and ("fullft" in train_modes or "lora" in train_modes):
                    ...
                    for h in handles:
                        _ = h.get()
                    _refresh_volume(f"stage-1 fullft training {stage.stage_id}")
                if "lora" in train_modes:
                    for model_name in train_models:
                        src = resolve_model_path_for_stage("fullft", model_name, stage.stage_id)
                        dst = model_path("lora", model_name, stage.stage_id)
                        if not os.path.exists(dst):
                            if not src:
                                expected = model_path("fullft", model_name, stage.stage_id)
                                raise RuntimeError(f"missing stage-1 base to copy into lora arm: {expected}")
                ...
                for h in handles:
                    _ = h.get()
                _refresh_volume(f"training {stage.stage_id}")
    ...
    _refresh_volume(f"pre-final-eval {final_stage}")
