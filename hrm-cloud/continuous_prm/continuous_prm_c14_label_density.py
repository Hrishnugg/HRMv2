...
class C14Config:
    ...
    mode: str = "full"
    cpu: bool = False
    # C14-R (2026-07-25 frozen design): independent world-set draws. 0 = the
    # original C14 streams; k>0 offsets ONLY the pool seed streams (test
    # cohorts, sampling rules, and everything keyed on `seed` are unchanged).
    draw_offset: int = 0
...
def _static_pool_seed(cfg: C14Config) -> int:
    ...
    return 14_000_000 + h + int(cfg.seed) + 1_000_000_000 * int(cfg.draw_offset)
...
def _dynamic_pool_seed(cfg: C14Config) -> int:
    ...
    return 14_250_000 + h + int(cfg.seed) + 1_000_000_000 * int(cfg.draw_offset)
...
def main(argv=None):
    ...
    p.add_argument("--draw-offset", type=int, default=0)
