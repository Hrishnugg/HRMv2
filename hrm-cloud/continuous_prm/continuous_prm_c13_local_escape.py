#!/usr/bin/env python3
"""C13-G: exact bounded local-escape heuristic ceiling.
...
radius plus a frontier bit for locally visible nodes that have an action
leaving the radius.  Local Dijkstra terminates at either the goal (if observed)
or a frontier node and adds Euclidean-to-goal as terminal cost.
...
class LocalEscapeConfig:
    ...
    out_dir: str = "runs/c13_local_escape"
...
def provider_name(radius_frac: float) -> str:
    return f"local_escape_r{float(radius_frac):.2f}".replace(".", "p")
...
def extract_local_observation(
    ...
    Outside endpoints are not exposed.  A local node receives only a frontier
    bit when one of its available actions leaves the observation radius.
    """
...
def exact_local_escape(observation: LocalObservation) -> Dict[str, Any]:
    ...
    while heap:
        ...
        if node == int(observation.goal_local):
            ...
            best = min(
                best,
                float(distance) + float(observation.euclid_to_goal[node]),
            )
...
def build_verdict(
    ...
    if primary_pass:
        verdict = "primary_exact_local_escape_gate_pass_development_only"
        authorization = "replicate_primary_radius_on_fresh_192_and_211_worlds"
    elif selected is not None:
        verdict = "alternate_local_escape_radius_found_development_only"
        authorization = "replicate_fixed_alternate_radius_on_fresh_worlds"
    else:
        verdict = "exact_local_escape_gate_fail"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="C13-G exact local-escape ceiling")
...
def main() -> None:
    ...
    verification = {
        ...
        "runtime_information": "radius_bounded_nodes_internal_edges_frontier_bits_goal_geometry",
    }
    ...
    manifest = {
        "experiment": "C13-G exact bounded local-escape heuristic ceiling",
        ...
        "model_loading_performed": False,
        "shortest_path_target": False,
        "target": "local_dijkstra_to_observed_frontier_plus_euclidean_terminal",
        "runtime_information": "radius_bounded_nodes_internal_edges_frontier_bits_goal_geometry",
