#!/usr/bin/env python3
"""
Average-heuristic + conditional basis-LoRA experiment for learned A* residual heuristics.

This script revises the clean transfer experiment to match the newer LoRA
specialization idea discussed in the March 26 meeting:

- First train one shared "average" heuristic model on the pooled training-task distribution.
- Then freeze that base model and add task-conditioned low-rank residual adapters.
- The adapters are expressed as a small set of LoRA basis matrices.
- A hypernetwork maps task properties to mixture coefficients over those bases.
- Eval remains sharded, cacheable, and preemption-safe under Modal.
...
APP_NAME = "transfer-astar-heuristic-avg-condlora-basis-v1"
VOLUME_NAME = os.environ.get("VOLUME_NAME", "transfer-astar-heuristic-avg-condlora-basis-v1-vol")
...
DATA_ROOT = "/data/transfer_astar_heuristic_avg_condlora_basis_v1"
...
def _run_tag() -> str:
    raw = os.environ.get("RUN_TAG") or "avg_condlora_basis_v1"
...
class CleanHeuristicModel(nn.Module):
    def __init__(self, cfg: BackboneConfig):
        ...
        self.cond_lora_enabled = False
        self.cond_lora_controller: Optional[nn.Module] = None
        self.cond_lora_modules = nn.ModuleList()
        self.cond_lora_task_desc_dim = 0
        self.cond_lora_num_basis = 0
        self.cond_lora_rank = 0
        self.cond_lora_controller_hidden = 0
        self.cond_lora_temperature = 1.0
        self.cond_lora_alpha = 0.0
    ...
    def enable_conditional_basis_lora(self, rank: int, alpha: float, num_basis: int, task_desc_dim: int,
                                      controller_hidden: int, temperature: float,
                                      include_conv: bool = True, include_attn: bool = True,
                                      init_scale: float = 0.01) -> int:
        if self.cond_lora_enabled:
            return len(self.cond_lora_modules)
        wrapped = _apply_conditional_basis_lora(
            self,
            rank=rank,
            alpha=alpha,
            num_basis=num_basis,
        ...
        self.cond_lora_controller = TaskToBasisController(task_desc_dim, controller_hidden, num_basis, temperature)
        self.cond_lora_modules = nn.ModuleList(_collect_conditional_lora_modules(self))
        self.cond_lora_enabled = True
        self.cond_lora_task_desc_dim = int(task_desc_dim)
        self.cond_lora_num_basis = int(num_basis)
        self.cond_lora_rank = int(rank)
        self.cond_lora_controller_hidden = int(controller_hidden)
        self.cond_lora_temperature = float(temperature)
        self.cond_lora_alpha = float(alpha)
        self.clear_active_task_descriptor()
        return wrapped

    def uses_conditional_lora(self) -> bool:
        return bool(self.cond_lora_enabled and self.cond_lora_controller is not None and len(self.cond_lora_modules) > 0)

    def clear_active_task_descriptor(self) -> None:
        if not self.uses_conditional_lora():
            return
        coeffs = torch.full(
            (self.cond_lora_num_basis,),
            1.0 / max(1, self.cond_lora_num_basis),
            device=next(self.parameters()).device,
            dtype=next(self.parameters()).dtype,
        )
        for mod in self.cond_lora_modules:
            mod.set_active_coeffs(coeffs)

    def compute_task_coeffs(self, task_desc: torch.Tensor) -> torch.Tensor:
        if not self.uses_conditional_lora():
            raise RuntimeError("conditional LoRA is not enabled")
        if task_desc.dim() == 1:
            task_desc = task_desc.unsqueeze(0)
        coeffs = self.cond_lora_controller(task_desc)
        if coeffs.dim() != 2:
            raise RuntimeError(f"unexpected coeff shape: {tuple(coeffs.shape)}")
        return coeffs.mean(dim=0)

    def set_task_descriptor(self, task_desc: torch.Tensor) -> torch.Tensor:
        if not self.uses_conditional_lora():
            return torch.empty(0, device=task_desc.device if isinstance(task_desc, torch.Tensor) else None)
        coeffs = self.compute_task_coeffs(task_desc)
        for mod in self.cond_lora_modules:
            mod.set_active_coeffs(coeffs)
        return coeffs
...
DYNAMICS_NAMES = ["static", "sparseDyn", "moderateDyn", "fullDyn"]
DYNAMICS_INDEX = {name: i for i, name in enumerate(DYNAMICS_NAMES)}
TASK_DESC_DIM = 6 + len(DYNAMICS_NAMES)


def _task_descriptor(size: int, max_steps: int, plan_horizon: int,
                     n_gates: int, n_patrollers: int, n_drifters: int,
                     dynamics: str) -> np.ndarray:
    dyn_onehot = np.zeros((len(DYNAMICS_NAMES),), dtype=np.float32)
    dyn_onehot[DYNAMICS_INDEX.get(dynamics, 0)] = 1.0
    base = np.array([
        float(size) / 256.0,
        float(max_steps) / 700.0,
        float(plan_horizon) / 50.0,
        float(n_gates) / 4.0,
        float(n_patrollers) / 4.0,
        float(n_drifters) / 4.0,
    ], dtype=np.float32)
    return np.concatenate([base, dyn_onehot], axis=0).astype(np.float32)


def task_descriptor_from_stage(stage: Stage) -> np.ndarray:
...
def task_descriptor_from_suite(suite: EvalSuite) -> np.ndarray:
    return _task_descriptor(
        size=suite.size,
        max_steps=suite.max_steps,
        plan_horizon=suite.plan_horizon,
        n_gates=suite.n_gates,
        n_patrollers=suite.n_patrollers,
        n_drifters=suite.n_drifters,
        dynamics=suite.dynamics,
    )


class ConditionalBasisWeightLoRA(nn.Module):
    def __init__(self, base_weight: torch.Tensor, rank: int, alpha: float, num_basis: int, init_scale: float = 0.01):
        super().__init__()
        self.rank = int(rank)
        ...
        self.register_buffer("default_coeffs", torch.full((self.num_basis,), 1.0 / max(1, self.num_basis), device=dev))
        self.register_buffer("residual_scale", torch.tensor(float(scale), device=dev))
        ...
        for _ in range(self.num_basis):
            ...
            self.As.append(a)
            self.Bs.append(b)
        self.active_coeffs: Optional[torch.Tensor] = None

    def set_active_coeffs(self, coeffs: torch.Tensor) -> None:
        self.active_coeffs = coeffs

    def clear_active_coeffs(self) -> None:
    ...
    def forward(self, base_w: torch.Tensor) -> torch.Tensor:
        ...
        coeffs = coeffs.to(device=out.device, dtype=out.dtype)
        scale = self.residual_scale.to(device=out.device, dtype=out.dtype)
        for idx in range(self.num_basis):
            out = out + scale * coeffs[idx] * (self.Bs[idx] @ self.As[idx])
        return out.reshape(self.weight_shape)


class TaskToBasisController(nn.Module):
    def __init__(self, task_desc_dim: int, hidden_dim: int, num_basis: int, temperature: float = 1.0):
        super().__init__()
        self.temperature = float(max(1e-6, temperature))
        self.net = nn.Sequential(
            nn.Linear(task_desc_dim, hidden_dim),
            nn.GELU(),
            nn.Linear(hidden_dim, hidden_dim),
            nn.GELU(),
    ...
    def forward(self, task_desc: torch.Tensor) -> torch.Tensor:
        logits = self.net(task_desc)
        return F.softmax(logits / self.temperature, dim=-1)


def _iter_lora_targets(module: nn.Module, include_conv: bool, include_attn: bool):
    for name, sub in module.named_modules():
        if name.startswith("cond_lora_controller"):
            continue
...
def _apply_conditional_basis_lora(module: nn.Module, rank: int, alpha: float, num_basis: int,
                                  include_conv: bool = True, include_attn: bool = True,
                                  init_scale: float = 0.01) -> int:
    wrapped = 0
    for _name, sub, attr in _iter_lora_targets(module, include_conv, include_attn):
        target_mod = sub
        target_attr = attr
        ...
        parametrize.register_parametrization(
            target_mod,
            target_attr,
            ConditionalBasisWeightLoRA(base_w.data, rank, alpha, num_basis, init_scale=init_scale),
            unsafe=True,
        )
        wrapped += 1
    return wrapped
...
def _collect_conditional_lora_modules(module: nn.Module) -> List[ConditionalBasisWeightLoRA]:
    out: List[ConditionalBasisWeightLoRA] = []
    for sub in module.modules():
        if isinstance(sub, ConditionalBasisWeightLoRA):
            out.append(sub)
    return out
...
def _set_hyplora_trainable(module: nn.Module, train_bias: bool = False) -> None:
    for name, p in module.named_parameters():
        p.requires_grad = False
        if ".parametrizations." in name:
            if ".As." in name or ".Bs." in name:
                p.requires_grad = True
        elif name.startswith("cond_lora_controller."):
            p.requires_grad = True
...
class TaskAwareStageEpisodeDataset(StageEpisodeDataset):
    def __init__(self, payload: Dict[str, Any], stage: Stage, history_len: int = HISTORY_LEN, patch_radius: int = PATCH_RADIUS):
        super().__init__(payload, history_len=history_len, patch_radius=patch_radius)
        self.stage = stage
        self.task_desc = torch.tensor(task_descriptor_from_stage(stage), dtype=torch.float32)

    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        item = super().__getitem__(idx)
        item["task_desc"] = self.task_desc
        return item
...
TRANSFER_MODES_ALL = ["avgft", "hyplora"]
...
TRAIN_TRANSFER_MODES_SPEC = _parse_models_spec(os.environ.get("TRAIN_TRANSFER_MODES") or "avgft,hyplora")
EVAL_TRANSFER_MODES_SPEC = _parse_models_spec(os.environ.get("EVAL_TRANSFER_MODES") or "avgft,hyplora")
...
def _normalize_modes(spec: Optional[List[str]]) -> List[str]:
    ...
    for x in spec:
        x = x.strip().lower()
        if x in ("full", "fullft", "finetune", "fine_tune", "avg", "avgft", "average", "base"):
            x = "avgft"
        if x in ("lora", "adapter", "adapters", "hyplora", "condlora", "conditional", "conditional_lora"):
            x = "hyplora"
...
LORA_TRAIN_BIAS = (_env_int("LORA_TRAIN_BIAS", 0) == 1)
LORA_ON_CONV = (_env_int("LORA_ON_CONV", 1) == 1)
LORA_ON_ATTN = (_env_int("LORA_ON_ATTN", 1) == 1)

CONDLORA_R = _env_int("CONDLORA_R", 0)
CONDLORA_NUM_BASIS = _env_int("CONDLORA_NUM_BASIS", max(4, len(STAGES_TO_RUN)))
CONDLORA_CONTROLLER_HIDDEN = _env_int("CONDLORA_CONTROLLER_HIDDEN", 64)
CONDLORA_CONTROLLER_TEMPERATURE = _env_float("CONDLORA_CONTROLLER_TEMPERATURE", 1.0)
POOLED_STAGE_ID = _sanitize_file_component(os.environ.get("POOLED_STAGE_ID") or "pooled_train")
AVG_EPOCHS = _env_int("AVG_EPOCHS", max((s.train_epochs for s in STAGES_TO_RUN), default=24))
CONDLORA_EPOCHS = _env_int("CONDLORA_EPOCHS", max((s.train_epochs for s in STAGES_TO_RUN), default=24))
...
def _save_model_artifact(path: str, cfg: BackboneConfig, arm: str, model_name: str, stage_id: str,
                         model: nn.Module, metrics: Dict[str, Any],
                         model_kind: str = "base", extra: Optional[Dict[str, Any]] = None) -> None:
    payload = {
        "cfg": asdict(cfg),
        "arm": arm,
...
def _stage_loader_map(stage_dataset_paths: Dict[str, str], device: str, task_aware: bool) -> Tuple[Dict[str, torch.utils.data.Dataset], Dict[str, torch.utils.data.DataLoader]]:
    datasets: Dict[str, torch.utils.data.Dataset] = {}
    loaders: Dict[str, torch.utils.data.DataLoader] = {}
    for stage in STAGES_TO_RUN:
        stage_id = stage.stage_id
        if stage_id not in stage_dataset_paths:
            continue
        payload = load_dataset(stage_dataset_paths[stage_id])
        ds: torch.utils.data.Dataset
        if task_aware:
            ds = TaskAwareStageEpisodeDataset(payload, stage)
        else:
            ds = StageEpisodeDataset(payload)
        datasets[stage_id] = ds
        loaders[stage_id] = torch.utils.data.DataLoader(
            ds,
    ...
    if not loaders:
        raise RuntimeError("no stage loaders built")
    return datasets, loaders


def _balanced_stage_batches(stage_ids: List[str], loaders: Dict[str, torch.utils.data.DataLoader],
                            datasets: Dict[str, torch.utils.data.Dataset]) -> Iterable[Tuple[str, Dict[str, torch.Tensor]]]:
    loader_iters = {sid: iter(loaders[sid]) for sid in stage_ids}
    steps_per_stage = max(1, max(math.ceil(len(datasets[sid]) / max(1, BATCH_SIZE)) for sid in stage_ids))
    for _ in range(steps_per_stage):
        stage_order = stage_ids[:]
        random.shuffle(stage_order)
        for sid in stage_order:
            try:
                batch = next(loader_iters[sid])
            except StopIteration:
                loader_iters[sid] = iter(loaders[sid])
                batch = next(loader_iters[sid])
            yield sid, batch


def _compute_batch_loss(model: CleanHeuristicModel, batch: Dict[str, torch.Tensor], device: str,
                        use_amp: bool, set_task_desc: bool = False) -> torch.Tensor:
    ...
    if set_task_desc:
        task_desc = batch["task_desc"].to(device)
        model.set_task_descriptor(task_desc)
...
def _rank_for_cfg(cfg: BackboneConfig) -> int:
    if CONDLORA_R > 0:
        return int(CONDLORA_R)
    return int(cfg.lora_rank)


def _train_avgft_impl(model_name: str, stage_dataset_paths: Dict[str, str], device: str, seed: int) -> Dict[str, Any]:
    vol.reload()
    _ensure_dirs()
    ...
    datasets, loaders = _stage_loader_map(stage_dataset_paths, device=device, task_aware=False)
    stage_ids = list(loaders.keys())
    ...
    ckpt_path = checkpoint_path("avgft", model_name, POOLED_STAGE_ID)
    final_path = model_path("avgft", model_name, POOLED_STAGE_ID)
    start_epoch, best_loss = _resume_training_checkpoint(ckpt_path, model, opt)
    epochs = max(1, AVG_EPOCHS)
    ...
    for epoch in range(start_epoch, epochs):
        epoch_loss = 0.0
        steps = 0
        for _sid, batch in _balanced_stage_batches(stage_ids, loaders, datasets):
            opt.zero_grad(set_to_none=True)
            loss = _compute_batch_loss(model, batch, device=device, use_amp=use_amp, set_task_desc=False)
            loss.backward()
            if GRAD_CLIP_NORM > 0:
        ...
        if is_best:
            ...
            _save_model_artifact(
                final_path, cfg, "avgft", model_name, POOLED_STAGE_ID, model,
                {
                    "best_loss": best_loss,
                    "epoch": epoch,
                    "arm": "avgft",
                    "train_stage_ids": stage_ids,
                },
                model_kind="avgft_base",
                extra={"train_stage_ids": stage_ids},
            )
        torch.save({"epoch": epoch, "model": model.state_dict(), "opt": opt.state_dict(), "best_loss": best_loss}, ckpt_path)
        vol.commit()
        print(f"[train][avgft][{model_name}] epoch {epoch+1}/{epochs} loss={epoch_loss:.6f} best={best_loss:.6f}")

    return {
        "ok": True,
        "arm": "avgft",
        "model_name": model_name,
        "stage_id": POOLED_STAGE_ID,
        "path": final_path,
        "best_loss": best_loss,
        "train_stage_ids": stage_ids,
    }
...
def _train_hyplora_impl(model_name: str, stage_dataset_paths: Dict[str, str], device: str, seed: int) -> Dict[str, Any]:
    vol.reload()
    _ensure_dirs()
    torch.manual_seed(seed)
    np.random.seed(seed)
    ...
    datasets, loaders = _stage_loader_map(stage_dataset_paths, device=device, task_aware=True)
    stage_ids = list(loaders.keys())

    cfg = MODEL_CONFIGS[model_name]
    base_path = source_model_path("avgft", model_name, POOLED_STAGE_ID)
    if not os.path.exists(base_path):
        raise FileNotFoundError(f"avgft base model missing for hyplora init: {base_path}")
    base_payload = _load_model_artifact(base_path, map_location="cpu")

    model = CleanHeuristicModel(cfg)
    model.load_state_dict(base_payload["model_state"], strict=False)
    rank = _rank_for_cfg(cfg)
    model.enable_conditional_basis_lora(
        rank=rank,
        alpha=LORA_ALPHA,
        num_basis=CONDLORA_NUM_BASIS,
        task_desc_dim=TASK_DESC_DIM,
        controller_hidden=CONDLORA_CONTROLLER_HIDDEN,
        temperature=CONDLORA_CONTROLLER_TEMPERATURE,
        include_conv=LORA_ON_CONV,
        include_attn=LORA_ON_ATTN,
        init_scale=LORA_INIT_SCALE,
    )
    model.to(device)

    _set_hyplora_trainable(model, train_bias=LORA_TRAIN_BIAS)
    params = _trainable_params(model)
    if not params:
        raise RuntimeError(f"no trainable parameters for hyplora/{model_name}")
    opt = torch.optim.AdamW(params, lr=LR_LORA, weight_decay=WEIGHT_DECAY_LORA)

    ckpt_path = checkpoint_path("hyplora", model_name, POOLED_STAGE_ID)
    final_path = model_path("hyplora", model_name, POOLED_STAGE_ID)
    start_epoch, best_loss = _resume_training_checkpoint(ckpt_path, model, opt)
    epochs = max(1, CONDLORA_EPOCHS)
    use_amp = (device == "cuda") and (_env_int("USE_AMP", 1) == 1)
    ...
    for epoch in range(start_epoch, epochs):
        epoch_loss = 0.0
        steps = 0
        coeff_summary: Dict[str, List[float]] = {}
        for sid, batch in _balanced_stage_batches(stage_ids, loaders, datasets):
            opt.zero_grad(set_to_none=True)
            loss = _compute_batch_loss(model, batch, device=device, use_amp=use_amp, set_task_desc=True)
            loss.backward()
            if GRAD_CLIP_NORM > 0:
                torch.nn.utils.clip_grad_norm_(params, GRAD_CLIP_NORM)
            opt.step()
            ...
            steps += 1
            with torch.no_grad():
                coeffs = model.compute_task_coeffs(batch["task_desc"].to(device))
                coeff_summary[sid] = coeffs.detach().float().cpu().tolist()
        epoch_loss /= max(1, steps)
        is_best = epoch_loss < best_loss
        if is_best:
            best_loss = epoch_loss
            _save_model_artifact(
                final_path, cfg, "hyplora", model_name, POOLED_STAGE_ID, model,
                {
                    "best_loss": best_loss,
                    "epoch": epoch,
                    "arm": "hyplora",
        ...
        torch.save({"epoch": epoch, "model": model.state_dict(), "opt": opt.state_dict(), "best_loss": best_loss}, ckpt_path)
        vol.commit()
        print(f"[train][hyplora][{model_name}] epoch {epoch+1}/{epochs} loss={epoch_loss:.6f} best={best_loss:.6f}")
...
def _train_model_impl(model_name: str, arm: str, stage_dataset_paths: Dict[str, str], device: str, seed: int) -> Dict[str, Any]:
    if arm == "avgft":
        return _train_avgft_impl(model_name, stage_dataset_paths, device=device, seed=seed)
    if arm == "hyplora":
        return _train_hyplora_impl(model_name, stage_dataset_paths, device=device, seed=seed)
...
def train_model_h100(model_name: str, arm: str, stage_dataset_paths: Dict[str, str], seed: int = 0) -> Dict[str, Any]:
    return _train_model_impl(model_name, arm, stage_dataset_paths, device="cuda", seed=seed)
...
def train_model_b200(model_name: str, arm: str, stage_dataset_paths: Dict[str, str], seed: int = 0) -> Dict[str, Any]:
    return _train_model_impl(model_name, arm, stage_dataset_paths, device="cuda", seed=seed)
...
def _load_model_for_eval(model_path_str: str, device: str) -> CleanHeuristicModel:
    ...
    model = CleanHeuristicModel(cfg)
    arm = payload.get("arm", "avgft")
    model_kind = payload.get("model_kind", "base")
    extra = payload.get("extra", {}) or {}
    if arm == "hyplora" or model_kind == "cond_basis_lora":
        model.enable_conditional_basis_lora(
            rank=int(extra.get("rank", _rank_for_cfg(cfg))),
            alpha=float(extra.get("lora_alpha", LORA_ALPHA)),
            num_basis=int(extra.get("num_basis", CONDLORA_NUM_BASIS)),
            task_desc_dim=int(extra.get("task_desc_dim", TASK_DESC_DIM)),
            controller_hidden=int(extra.get("controller_hidden", CONDLORA_CONTROLLER_HIDDEN)),
            temperature=float(extra.get("temperature", CONDLORA_CONTROLLER_TEMPERATURE)),
            include_conv=LORA_ON_CONV,
            include_attn=LORA_ON_ATTN,
            init_scale=LORA_INIT_SCALE,
        )
...
def _validation_suite_ids(_stage_id: str = POOLED_STAGE_ID) -> List[str]:
    out: List[str] = []
    for stage in STAGES_TO_RUN:
        sid = f"ID_A{stage.size}_{stage.dynamics}"
        if sid in EVAL_SUITE_BY_ID and sid not in out:
            out.append(sid)
        if stage.size == 64:
            for fam in ("B", "C"):
                ood_sid = f"OOD_{fam}{stage.size}_{stage.dynamics}"
                if ood_sid in EVAL_SUITE_BY_ID and ood_sid not in out:
                    out.append(ood_sid)
    return out
...
def run_policy_episode(suite: EvalSuite, seed: int, model: Optional[CleanHeuristicModel], alpha: float,
    ...
    if model is not None and model.uses_conditional_lora():
        task_desc = torch.from_numpy(task_descriptor_from_suite(suite)).to(device)
        with torch.no_grad():
            model.set_task_descriptor(task_desc)
...
def resolve_model_path_for_stage(arm: str, model_name: str, stage_id: str = POOLED_STAGE_ID) -> str:
...
def _alpha_for_model_stage(arm: str, model_name: str, stage_id: str = POOLED_STAGE_ID) -> float:
...
def run_pipeline(train_models: List[str], eval_models: List[str], train_modes: List[str], eval_modes: List[str],
    ...
    _ensure_dirs()
    print("=" * 78)
    print("AVERAGE + CONDITIONAL BASIS-LORA A* EXPERIMENT")
    print("=" * 78)
    ...
    print(f"Hyplora epochs:  {CONDLORA_EPOCHS}")
    print(f"Eval suites:     {[s.suite_id for s in EVAL_SUITES]}")
    print(f"Budgets:         {EVAL_BUDGETS}")
    print(f"Alpha candidates:{ALPHA_CANDIDATES}")
    print(f"CondLoRA basis:  {CONDLORA_NUM_BASIS}")
    print("")

    stage_dataset_paths: Dict[str, str] = {}

    for stage in STAGES_TO_RUN:
        print(f"\n📦 Stage {stage.stage_id}: dataset collection")
        ...
        stage_dataset_paths[stage.stage_id] = merged_path

    if SKIP_TRAIN:
        print("\n⏭️  SKIP_TRAIN=1 set; skipping training.")
    else:
        if "avgft" in train_modes:
            handles = []
            for model_name in train_models:
                dst = model_path("avgft", model_name, POOLED_STAGE_ID)
                if os.path.exists(dst):
                    print(f"  ✓ existing model: {dst}")
                    continue
                fn = _get_train_fn(model_name)
                handles.append(fn.spawn(model_name, "avgft", stage_dataset_paths, seed_base + 10))
                if len(handles) >= max_parallel_train:
                    _ = handles.pop(0).get()
            for h in handles:
                _ = h.get()
            _refresh_volume("avgft training")

        if "hyplora" in train_modes:
            handles = []
            for model_name in train_models:
                dst = model_path("hyplora", model_name, POOLED_STAGE_ID)
                if os.path.exists(dst):
                    print(f"  ✓ existing model: {dst}")
                    continue
                avg_path = resolve_model_path_for_stage("avgft", model_name, POOLED_STAGE_ID)
                if not avg_path:
                    raise RuntimeError(f"hyplora requested but avgft base missing for {model_name}")
                fn = _get_train_fn(model_name)
                handles.append(fn.spawn(model_name, "hyplora", stage_dataset_paths, seed_base + 20))
                if len(handles) >= max_parallel_train:
                    _ = handles.pop(0).get()
            for h in handles:
                _ = h.get()
            _refresh_volume("hyplora training")
    if SKIP_ALPHA_TUNE:
        print("\n⏭️  SKIP_ALPHA_TUNE=1 set; skipping alpha tuning.")
    else:
        val_suite_ids = _validation_suite_ids(POOLED_STAGE_ID)
        ...
        for model_name in tune_models:
            for arm in tune_modes:
                mpath = resolve_model_path_for_stage(arm, model_name, POOLED_STAGE_ID)
                if not mpath:
                    continue
                ap = alpha_path(arm, model_name, POOLED_STAGE_ID)
                ...
                for alpha in ALPHA_CANDIDATES:
                    for suite_id in val_suite_ids:
                        jobs.append({
                            "model_eval_id": eval_model_id(arm, model_name, POOLED_STAGE_ID),
                ...
                for alpha, vals in grouped.items():
                    ...
                    if best_key is None or key > best_key:
                        best_key = key
                        best_alpha = alpha
                _save_alpha_choice(arm, model_name, POOLED_STAGE_ID, best_alpha, score_payload)
                vol.commit()
                print(f"  ✓ tuned alpha [{arm}][{model_name}] = {best_alpha}")
    ...
    print(f"\n📊 Final evaluation on pooled models (parallel, sharded + cacheable)")
    eval_jobs: List[Dict[str, Any]] = []

    for suite in EVAL_SUITES:
        for budget in EVAL_BUDGETS:
            eval_jobs.append({
                "model_eval_id": eval_model_id("baseline", "manhattan_astar", POOLED_STAGE_ID),
    ...
    for model_name in eval_models:
        for arm in eval_modes:
            mpath = resolve_model_path_for_stage(arm, model_name, POOLED_STAGE_ID)
            if not mpath:
                print(f"  ! missing model {arm}/{model_name}, skipping eval")
                continue
            alpha = _alpha_for_model_stage(arm, model_name, POOLED_STAGE_ID)
            for suite in EVAL_SUITES:
                for budget in EVAL_BUDGETS:
                    eval_jobs.append({
                        "model_eval_id": eval_model_id(arm, model_name, POOLED_STAGE_ID),
    ...
    results_json = f"{RESULTS_DIR}/final_results__{POOLED_STAGE_ID}.json"
    results_csv = f"{RESULTS_DIR}/final_results__{POOLED_STAGE_ID}.csv"
    _write_json_atomic(results_json, {"rows": rows, "pooled_stage_id": POOLED_STAGE_ID, "run_tag": RUN_TAG, "model_run_tag": MODEL_RUN_TAG})
