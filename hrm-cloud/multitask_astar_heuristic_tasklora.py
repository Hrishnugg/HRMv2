#!/usr/bin/env python3
"""
Multitask average-heuristic + oracle task-LoRA experiment for learned A* residual heuristics.
...
APP_NAME = "multitask-astar-heuristic-tasklora-v1"
VOLUME_NAME = os.environ.get("VOLUME_NAME", "multitask-astar-heuristic-tasklora-v1-vol")
...
DATA_ROOT = "/data/multitask_astar_heuristic_tasklora_v1"
...
class BalancedMultiStageDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, payloads_by_stage: Dict[str, Dict[str, Any]],
        ...
        self.max_spatial = max(
            ds.ep_cache[0]["size"] for ds in self.datasets.values() if ds.ep_cache
        )
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        obs = item["obs_seq"]
        s = obs.shape[-1]
        if s < self.max_spatial:
            padded = torch.zeros(*obs.shape[:-2], self.max_spatial, self.max_spatial, dtype=obs.dtype)
            padded[..., :s, :s] = obs
            item["obs_seq"] = padded
        return item
...
def run_pipeline(train_models: List[str], eval_models: List[str], train_arms: List[str], eval_arms: List[str],
    ...
    results_json = f"{RESULTS_DIR}/final_results__multitask_tasklora.json"
    results_csv = f"{RESULTS_DIR}/final_results__multitask_tasklora.csv"
