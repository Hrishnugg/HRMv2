...
VOLUME_NAME = "transfer-astar-heuristic-imitation-v2-vol-v2"
...
# -----------------------------
# LoRA adapters (stage-wise)
# -----------------------------

class LoRAAdapter(nn.Module):
    """
    A single low-rank adapter for a weight matrix flattened as (out_features, in_features).

    We use the common init where B is zero, so the initial delta is exactly zero and
    loading a previous-stage checkpoint produces identical behavior before training.
    """
    def __init__(self, out_features: int, in_features: int, r: int, alpha: float, init_scale: float = 0.01):
        super().__init__()
        ...
        self.A = nn.Parameter(torch.randn(self.r, self.in_features) * float(init_scale))
...
class LoRAParametrization(nn.Module):
    """
    Parametrization that replaces a parameter W with:

        W_eff = W + Σ_i ΔW_i

    where each ΔW_i is a trainable low-rank adapter. This supports "stacking"
    one adapter per curriculum stage.
    """
    def __init__(self, weight_shape: torch.Size, r: int, alpha: float, n_adapters: int, init_scale: float = 0.01):
        super().__init__()
        if len(weight_shape) < 2:
            raise ValueError(f"LoRA expects a matrix-like weight, got shape={tuple(weight_shape)}")

        self.weight_shape = tuple(int(d) for d in weight_shape)
        self.out_features = int(weight_shape[0])
        in_features = 1
        ...
        self.ensure_n_adapters(int(n_adapters))

    def ensure_n_adapters(self, n: int) -> None:
        if n < 0:
            raise ValueError("n_adapters must be >= 0")
        while len(self.adapters) < n:
            self.adapters.append(LoRAAdapter(self.out_features, self.in_features, self.r, self.alpha, self.init_scale))

    def set_trainable_adapter(self, adapter_idx: int) -> None:
        # Freeze all adapters except the requested one.
        for i, ad in enumerate(self.adapters):
            req = (i == adapter_idx)
            ad.A.requires_grad = req
            ad.B.requires_grad = req

    def forward(self, W: torch.Tensor) -> torch.Tensor:
        if self.r <= 0 or len(self.adapters) == 0:
            return W

        W2 = W.reshape(self.out_features, self.in_features)
        delta = None
        for ad in self.adapters:
            d = ad.delta()
            delta = d if delta is None else (delta + d)

        return (W2 + delta).reshape(self.weight_shape)

def _register_lora_param(module: nn.Module, param_name: str, r: int, alpha: float, n_adapters: int, init_scale: float) -> None:
    ...
    if parametrize.is_parametrized(module, param_name):
        # If already parametrized (e.g., for few-shot where we add an extra adapter),
        # try to find our parametrization and extend adapter count.
        plist = getattr(module, "parametrizations", None)
        if plist is None or param_name not in plist:
            return
        for pmod in plist[param_name]:
            if isinstance(pmod, LoRAParametrization):
                pmod.ensure_n_adapters(n_adapters)
                return
        return
    ...
    parametrize.register_parametrization(
        module,
        param_name,
        LoRAParametrization(w.shape, r=r, alpha=alpha, n_adapters=n_adapters, init_scale=init_scale),
    )
...
def inject_lora(
    ...
    """
    Inject LoRA parametrizations in-place.

    Targets:
      - nn.Linear.weight
      - nn.Conv2d.weight (optional)
      - nn.GRU weight_ih_l{k} / weight_hh_l{k} (optional)
      - nn.GRUCell weight_ih / weight_hh (optional)
    """
    if r <= 0 or n_adapters <= 0:
        return
    ...
    for m in model.modules():
        if isinstance(m, nn.Linear):
            _register_lora_param(m, "weight", r, alpha, n_adapters, init_scale)
        elif enable_conv and isinstance(m, nn.Conv2d):
            _register_lora_param(m, "weight", r, alpha, n_adapters, init_scale)

        if enable_gru and isinstance(m, nn.GRU):
            for l in range(int(m.num_layers)):
                _register_lora_param(m, f"weight_ih_l{l}", r, alpha, n_adapters, init_scale)
                _register_lora_param(m, f"weight_hh_l{l}", r, alpha, n_adapters, init_scale)
        elif enable_gru and isinstance(m, nn.GRUCell):
            _register_lora_param(m, "weight_ih", r, alpha, n_adapters, init_scale)
            _register_lora_param(m, "weight_hh", r, alpha, n_adapters, init_scale)

def set_lora_trainable(model: nn.Module, adapter_idx: int, train_bias: bool = False) -> None:
    """
    Freeze everything, then unfreeze ONLY the LoRA parameters for `adapter_idx`.
    Optionally also unfreeze any bias terms (BitFit-style).
    """
    for p in model.parameters():
        p.requires_grad = False

    for m in model.modules():
        plist = getattr(m, "parametrizations", None)
        if plist is None:
            continue
        for _pname, p_list in plist.items():
            for pmod in p_list:
                if isinstance(pmod, LoRAParametrization):
                    pmod.set_trainable_adapter(adapter_idx)
...
class ModelConfig:
    ...
    lora_n_adapters: int = 0
    lora_init_scale: float = 0.01
    lora_on_conv: bool = True
    lora_on_gru: bool = True
...
def build_model(cfg: ModelConfig) -> HeuristicModel:
    ...
    # Optional LoRA injection (enables stage-wise adapter stacking)
    if cfg.lora_r > 0 and cfg.lora_n_adapters > 0:
        inject_lora(
            model,
            r=cfg.lora_r,
            alpha=cfg.lora_alpha,
            n_adapters=cfg.lora_n_adapters,
            init_scale=cfg.lora_init_scale,
            enable_conv=cfg.lora_on_conv,
            enable_gru=cfg.lora_on_gru,
        )
...
def _train_model_impl(model_name: str, stage_id: str, dataset_path: str, device: str, seed: int) -> Dict[str, Any]:
    ...
    if use_lora:
        ...
        lora_on_gru = (_env_int("LORA_ON_GRU", 1) == 1)

        # One adapter per curriculum stage.
        n_adapters = stage_idx + 1

        cfg = replace(
            cfg,
            lora_r=lora_r,
            lora_alpha=lora_alpha,
            lora_n_adapters=n_adapters,
            lora_init_scale=lora_init_scale,
            lora_on_conv=lora_on_conv,
            lora_on_gru=lora_on_gru,
        )
    ...
    if use_lora:
        ...
        train_base = train_base_stage1 if stage_idx == 0 else train_base_later
        if train_base:
            for p in model.parameters():
                p.requires_grad = True
        else:
            # Freeze base + older adapters; train ONLY this stage's adapter.
            set_lora_trainable(model, adapter_idx=stage_idx, train_bias=train_bias)

        opt_params = [p for p in model.parameters() if p.requires_grad]
        if not opt_params:
            raise RuntimeError("USE_LORA=1 but no trainable parameters were selected. Check adapter_idx/env flags.")
    else:
...
def run_pipeline(only_models, max_parallel_train, max_parallel_collect, max_parallel_eval,
    ...
    if not skip_fewshot:
        ...
        if _env_int("EVAL_EXISTING_FEWSHOT", 1) == 1:
            for m in eval_model_names:
                for k in fewshot_ks:
